...
export default function PropertyFinderDetailV2({ project, anonymous = false }: PropertyFinderDetailProps) {
  ...
  useEffect(() => {
    ...
    if (coords && (coords.lat || coords.latitude)) {
      lng = Number(coords.lon || coords.lng || coords.longitude) || lng;
      lat = Number(coords.lat || coords.latitude) || lat;
    } else if (project.location && (project.location as any).coordinates) {
      const pCoords = (project.location as any).coordinates;
      lng = Number(pCoords.lon || pCoords.lng || pCoords.longitude) || lng;
      lat = Number(pCoords.lat || pCoords.latitude) || lat;
  ...
  const toggleMapStyle = () => {
    ...
    map.current.setStyle(newStyle === 'satellite' ? 'mapbox://styles/mapbox/satellite-v9' : 'mapbox://styles/mapbox/light-v11');
  };
                ...
                <div className={styles.facilitiesList}>
                  {amenities.map((amenity: any, idx: number) => {
                    ...
                    if (typeof amenity === 'string') {
                      ...
                      displayStr = getLocalizedText(amenity) || amenity.name || JSON.stringify(amenity);
                    }
                    
                    if (displayStr && typeof displayStr === 'string' && displayStr.includes('-')) {
                      displayStr = displayStr.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
                    }
                    return (
